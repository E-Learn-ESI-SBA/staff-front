...
export default function Awards() {
    return (
     <div className='flex flex-col gap-4 border-t border-[#EAEAEA] py-4 ' >
         ...
         <Trophy />
        <h1 className="text-[#121212] font-medium text-xl  " >Honers & Awards</h1>
        </div>

{awards.map((award,i)=>
   <div className='flex flex-col gap-1 items-start'  key={i} >
   <p className="text-[#121212] font-medium text-lg  " > {award.category}</p>
   <p className=' text-[#807E7E] ' > {award.name}</p>
    <p className=' text-[#807E7E] text-sm '  > {award.date}</p>
  <p className=' font-light text-sm text-black text-[#807E7E] ' >{award.description}</p>
   </div>
