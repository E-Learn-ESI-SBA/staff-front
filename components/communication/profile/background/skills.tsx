...
// export default function Skills({skills,other_skills}: Props) {
    ...
    <div key={i} className="card flex flex-col items-center justify-center h-48 w-48 justify-start  ">
        <div className="percent relative">
        <svg width="120" height="120" className="relative" >
                  <circle cx="60" cy="60" r="55"></circle>
                  <circle cx="60" cy="60" r="55" style={{"--percent": skill.percent }}></circle>
                </svg>
...
<p className="text-[#3E3E59] font-semibold text-lg  " >Vishnu also knows about</p>
<ul className="flex justify-start items-center gap-4" >

    {profile_skills.other_skills.map((skill,i)=>
    <li className="bg-[#A7A7A7] text-white p-2 rounded-lg text-xs "  key={i}> {skill} </li>  
    )}
</ul>
     </div>
    )
