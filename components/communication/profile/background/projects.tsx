...
<p className="text-[#3E3E59] font-medium text-lg  " > {project.name}</p>
<div className=' font-light text-base text-[#807E7E] ' >
 <span> {project.start_date}</span>-<span> {project.end_date}</span>
</div>
<p className=' font-light text-sm text-black text-[#807E7E] ' > {project.description}</p>
<p className="font-medium text-lg  " >Team Members</p>
<ul className='flex justify-start gap-2' >
    {project.team_members.map((member,i)=>
 <li key={i} > <Link href={member.link} >   <Image src={member.image}  alt='profile pic' width={0} height={0} sizes="100vw" className="w-12 h-12 rounded-full"  /> </Link></li>    
    )}
</ul>
</div>
