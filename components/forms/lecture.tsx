    ...
    const submitHandler = async (v: TLectureSchema) : Promise<void> => {
        try {
            ...
            onSubmit(prev => {
                ...
                if ((!courseIndex || !sectionIndex || !lectureIndex) || lectureIndex == -1 || sectionIndex == -1 || courseIndex == -1 ) return prev
                const newPrev = {...prev}
                newPrev.courses[courseIndex].sections[sectionIndex].lectures[lectureIndex].name = v.name
                newPrev.courses[courseIndex].sections[sectionIndex].lectures[lectureIndex].groups = v.groups.map(g => g.value)
                return newPrev
            } )
