...
const Navbar = () => {
  ...
  return (
    <div className={`w-full px-4 md:px-8  `}>
      <div
        className={`flex justify-between items-center py-4  ${path.substring(1) ? "text-courses-main" : "text-white  border-white border-b border-dashed "} `}
      >
        ...
        <div className="hidden sm:flex justify-between items-center gap-4 ">
          {Links.map((link, index) => (
            <Link
              key={index}
              href={link.url}
              className={`${isActiveLink(link.url) ? "font-semibold" : ""}`}
            >
              {link.name}
            </Link>
          ))}
