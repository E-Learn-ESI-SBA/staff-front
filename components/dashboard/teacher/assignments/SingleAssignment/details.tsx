...
const Details: React.FC<QuizDetailsProps> = ({
  ...
  <table className="w-full border-collapse border border-gray-300 max-w-96 ">
    <thead>
      <tr>
        <th className="py-2 px-4 bg-gray-100 border border-gray-300">File Name</th>
        <th className="py-2 px-4 bg-gray-100 border border-gray-300">File URL</th>
      </tr>
    </thead>
    <tbody>
      {files.map((file, index) => (
        <tr key={index}>
          <td className="py-2 px-4 border border-gray-300">{file.name}</td>
          <td className="py-2 px-4 border border-gray-300">
            <a href={file.url} className="text-blue-500">{file.url}</a>
          </td>
        </tr>
      ))}
    </tbody>
