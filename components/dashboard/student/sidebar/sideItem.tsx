...
export default function SideItem({ label, icon, url }: TProps) {
  ...
  return (
    <Link href={url} className=''>
      <Button className={`flex ${isActive ? 'bg-blue-500 text-white' : 'text-text bg-transparent'}  w-60  py-6 flex justify-start hover:bg-blue-500 hover:text-white gap-4 text-xl`}>
        {SideIcon(icon)}
        <p>{label}</p>
      </Button>
    </Link>
