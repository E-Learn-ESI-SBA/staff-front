...
export default function Questions({form,index }) {
        ...
        <FormField
							...
							render={({ field }) => (
								<FormItem className="w-full">
									<FormControl>
										<Input type="text" placeholder='Type Question Here '  {...field} />
									</FormControl>
									<FormMessage />
								</FormItem>
							)}
                                    ...
                                    onChange={(e) =>  {
                                        if (e.target.files) {
                                          ...
                                          reader.onloadend = () => {
                                                ...
                                                form.setValue(`questions.${index}.qst_image`, reader.result as string)
                                            };                                       
                                            try {
 ...
 {form.formState?.errors.questions && (
  <>
    ...
    {Object.keys(form.formState.errors.questions).map((key, indexError) => (
     <p key={indexError}> {key == index &&   <TypographyP className="indent-6 text-red-600 my-5 text-sm">
     {form.formState.errors.questions[key].answers?.root?.message}
     </TypographyP>   }    </p>
    ))}
  </>
)}
