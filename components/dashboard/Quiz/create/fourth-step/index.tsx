      ...
      <div className="p-2" >
        ...
        {third_step_content.questions.map((question, index) => (
          <div key={index} className="flex flex-col my-8 gap-4">
            ...
            {question.qst_image && <Image src={question.qst_image} alt={question.qst_title} width={0} height={0} sizes="100vw" className="w-full max-w-96 h-64" />}
            <ul className="flex flex-col gap-4 text-medium">
              {question.answers.map((answer, i) => (
                <li key={i} className={`flex justify-between items-center p-2 border-b-2 border-[#EFEFEF] ${answer.validite ? '' : 'text-[#8A8A8A]'}`}>
                  <p>
                    <span>{i + 1}. </span>
                    <span>{answer.title}</span>
                  </p>
                  {answer.validite && <div className="px-4 py-2 rounded-lg text-[#2E8760] bg-[#E9FFF5]">Marked as Correct</div>}
                </li>
              ))}
            </ul>
          </div>
