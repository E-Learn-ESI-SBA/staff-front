...
export function Filter<T>({filters,withSearch,data,setFilteredData}:Props<T>) {
    // Search State
   ...
   // const [selectedFilters,setSelectedFilters] = useState<Header<T>[]>([]);
    ...
    const SearchCmp = () => {
        return (
                <div className="relative">
                    ...
                    <Input  placeholder="Search" className="pl-8" onChange={e => searchHandler(e)} />
                </div>
        )
    }
        ...
        return (
            <Select onValueChange={v => filterHandler(filter.key as string ,v)}>
                ...
                <SelectContent>
                    ...
                    {data.map((item) => (
                        <SelectItem  value={item[filter.key] as unknown as string} key={item[filter.key].toString()}>
    ...
    const dataFilterHandler = () => {
        let filteredData = data.filter(item => {
            if(search !== '' && !Object.values(item).some(val => val.toString().toLowerCase().includes(search.toLowerCase()))) {
                return false;
            }
    ...
    return (
        <div className="flex items-start space-x-4 justify-between gap-4">
            ...
            <Button onClick={dataFilterHandler} className="px-4 py-2  rounded-md">Filter</Button>
            </div>
        </div>
    )
